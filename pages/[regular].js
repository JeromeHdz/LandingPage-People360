...
export const getStaticPaths = async () => {
  const regularSlugs = getSinglePage("blog/content");
  const postSlugs = getSinglePage(`blog/content/${blog_folder}`);
...
export const getStaticProps = async ({ params }) => {
  ...
  const getPostSlug = getSinglePage(`blog/content/${blog_folder}`);
  const postSlug = getPostSlug.map((item) => item.slug);
  // aughor data
  const authors = getSinglePage("blog/content/authors");
  // all single pages
  const posts = getSinglePage(`blog/content/${blog_folder}`);
