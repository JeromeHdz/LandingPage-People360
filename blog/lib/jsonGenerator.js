...
// get post data
const getPosts = fs.readdirSync(path.join(`blog/content/${blog_folder}`));
...
const posts = filterPosts.map((filename) => {
  ...
  const postData = fs.readFileSync(
    path.join(`blog/content/${blog_folder}/`, filename),
    "utf-8"
  );
...
try {
  fs.writeFileSync(`blog/json/posts.json`, JSON.stringify(posts));
} catch (err) {
  console.error(err);
}
